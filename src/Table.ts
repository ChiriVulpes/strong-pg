...
export default class Table<TABLE extends TableSchema, DATABASE extends DatabaseSchema, NAME extends DatabaseSchema.TableName<DATABASE>> {
	...
	public as<ALIAS extends string> (alias1: ALIAS) {
		return {
			innerJoin: <TABLE_NAME extends DatabaseSchema.TableName<DATABASE>, TABLE2_ALIAS extends string = TABLE_NAME> (tableName: TABLE_NAME, alias2?: TABLE2_ALIAS): Join<TABLE, DatabaseSchema.Table<DATABASE, TABLE_NAME>, NAME, TABLE_NAME, ALIAS, TABLE2_ALIAS> => {
				return new Join("INNER", this.name, tableName, alias1, alias2);
			},
		}
	}

	public innerJoin<TABLE_NAME extends DatabaseSchema.TableName<DATABASE>, ALIAS extends string = TABLE_NAME> (tableName: TABLE_NAME, alias?: ALIAS) {
		return new Join<TABLE, DatabaseSchema.Table<DATABASE, TABLE_NAME>, NAME, TABLE_NAME, NAME, ALIAS>("INNER", this.name, tableName, undefined, alias);
	}
}
